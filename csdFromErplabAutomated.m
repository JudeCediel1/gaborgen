% This script was not written by me (Chris Panitz), only adapted to take
% function instead of GUI input
%
...
function [EEG, erpcom] = csdFromErplabAutomated(EEG, m, lambda, radius)
if nargin == 1
    csd_param = [4, 1e-5, 10]; % default in ERPLAB GUI
elseif nargin == 4
    csd_param = [m, lambda, radius];
else
    error('please enter values for all CSD parameters (m, lambda, radius) or none for default (4, 1e-5, 10)')
end
